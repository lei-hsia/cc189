##### Q: time complexity of sorting a string using COMPARISION SORT: O(N logN). (there're algorithms that has a smaller big-O but those are not general cases, not discussed here)
